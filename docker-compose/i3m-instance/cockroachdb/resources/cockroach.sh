...
setup_certs_dir() {
  ...
  # Create certificates for i3-MARKET specific users
  for user in auditable_accounting cloud_wallet conflict_resolution; do
    if [ ! -e "$certs_dir"/client."$user"_node1.key -a ! -e "$certs_dir"/client."$user"_node1.crt ]; then
      echo "Creating certificate for user: $user"
      $cockroach_entrypoint cert create-client --certs-dir="$certs_dir" \
      --ca-key="$certs_dir"/ca.key "$user"_node1
    fi
  done
